...
//         format: 'uuid',
//         ...options,
//         isArray: !!options.each, // FIX: ensure isArray is boolean, not undefined
//       }),
//     );
//   }
...
export function EnumField<TEnum extends object>(
  ...
  if (options.swagger !== false) {
    // Ensure required is always boolean and isArray is always boolean
    const { required, each, ...restOptions } = options;
    decorators.push(
      ApiProperty({
        type: 'string',
        enum: getEnum(),
